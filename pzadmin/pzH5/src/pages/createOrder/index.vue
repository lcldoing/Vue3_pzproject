<template>
  <div class="container">
    ...
    <van-cell class="cell">
        <template #title>
            <van-image :src="createInfo.service.serviceImg" radius="5" width="25" height="25">
            </van-image>
    ...
    <van-cell class="cell">
      <template #title>就诊时间 </template>
        <template #default>
          <div  @click="showStarTime=true">{{currentDate||"请选择就诊时间"}}
            <van-icon name="arrow"></van-icon>
          </div>
        </template>
    </van-cell>   
    ...
    <van-cell class="cell">
      <template #title>陪诊师 </template>
        <template #default>
          <div  @click="showComponion=true">{{companionName||"请选择陪护师"}}
            <van-icon name="arrow"></van-icon>
          </div>
        </template>
    </van-cell>   
...
const createInfo=reactive({
    companion:[{text:'小王',id:'1'},{text:'黑黑',id:'2'}],
    hospitals:[],
    service:{}
})
...
const {hospitals,service}=data.data
Object.assign(createInfo,{hospitals,service})
// Object.assign(createInfo,data.data) 由于数值没有办法获取，需要对其中的内容重新赋值
})
...
const showComponionConfirm=(item)=>{
  ...
  console.log(typeof(form.companion_id),'form.companion_id');
